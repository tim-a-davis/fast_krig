...
class Log:
    def __init__(self, index: np.ndarray, streams: dict, x_coord: float=None, y_coord: float=None):
        self.index = index
        ...
        wrap_debug(self)
    @classmethod
    def from_numpy(cls, log_arr: np.ndarray, depth: str=None, labels: list=None, x_coord: float=None, y_coord: float=None):
        if not depth:
        ...
        streams = {label: arr for label, arr in zip([l for l in labels if l != depth], log_arr.T)}
